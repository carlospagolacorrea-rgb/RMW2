...
const AuthModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook') => {
    ...
    // No need to setSocialLoading(null) on success as it redirects
  };
      ...
      <div className="w-full max-w-md border-2 border-amber-500 p-8 glass-effect relative shadow-[0_0_50px_rgba(255,188,71,0.2)]">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <button
            ...
            className={`retro-button py-3 text-[10px] flex items-center justify-center gap-2 transition-all ${socialLoading === 'google' ? 'opacity-50' : ''}`}
          >
          ...
          <button
            ...
            disabled={!!socialLoading || loading}
            className={`retro-button py-3 text-[10px] flex items-center justify-center gap-2 transition-all ${socialLoading === 'facebook' ? 'opacity-50' : ''}`}
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          {error && <p className="crt-text text-[10px] text-red-500 uppercase text-center border border-red-500/20 py-2 bg-red-500/5 animate-pulse">{error}</p>}

          <button type="submit" disabled={loading || !!socialLoading} className="retro-button w-full py-4 text-xl uppercase mt-4">
